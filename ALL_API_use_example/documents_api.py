...
def get_documents_paginated(base_url, page=1, page_size=10, headers=None):
    ...
    try:
        response = requests.post(
            f"{base_url}/documents/paginated", headers=headers, json=data
        )
...
def scan_new_documents(base_url, headers=None):
    ...
    print("\n=== 扫描新文档 ===")
    try:
        response = requests.post(f"{base_url}/documents/scan", headers=headers)
        ...
        if response.status_code == 200:
            ...
            if "errors" in result:
                ...
                for error in result["errors"]:
                    print(
                        f"- {error.get('file_path', '未知文件')}: {error.get('error', '未知错误')}"
                    )
